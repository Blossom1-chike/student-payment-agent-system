...
function ChatInterface() {
    ...
    const onSubmit = async (e)=>{
        ...
        if (attachedFile) userMessageContent = attachedFile.name + (inputValue ? `: ${inputValue}` : "");
        const userMessage = {
        ...
        setIsTyping(true);
        setInputValue(""); // Clear text input immediately
        if (attachedFile) setUploading(true);
        try {
            const textToSend = attachedFile ? inputValue || "Uploaded ID image." : inputValue;
            const data = await sendChat(textToSend, state, threadId, attachedFile);
            // Replace '/api/your-backend-endpoint' with your actual backend API URL
...
//# sourceMappingURL=_8f396e15._.js.map